...
==============================
Developing dodiscover learners
==============================

Whether you are proposing an learner for inclusion in dodiscover,
developing a separate package compatible with dodiscover, or
implementing custom components for your own projects, this chapter
details how to develop objects that safely interact with dodiscover
tools.
...
Elements of the dodiscover API are described more definitively in the
:ref:`glossary`.

Different objects
-----------------

The main objects in dodiscover are (one class can implement
multiple interfaces):

:Learner:

    The base object, implements a ``fit`` method to learn from data and causal context::

      learner = learner.fit(data, context)
...
The API has one predominant object: the learner. An learner is an
object that fits a model based on some training data and is capable of
inferring some properties on new data. It can be, for instance, a
classifier or a regressor. All learners implement the fit method::

    learner.fit(X, y)

All built-in learners also have a ``set_params`` method, which sets
data-independent parameters (overriding previous parameter values passed
to ``__init__``).

All learners in the main dodiscover codebase should inherit from
``dodiscover.base.Baselearner``.

Instantiation
^^^^^^^^^^^^^

This concerns the creation of an object. The object's ``__init__`` method
might accept constants as arguments that determine the learner's behavior
...
an learner without passing any arguments to it. The arguments should all
correspond to hyperparameters describing the model or the optimisation
problem the learner tries to solve. These initial arguments (or parameters)
are always remembered by the learner.
Also note that they should not be documented under the "Attributes" section,
but rather under the "Parameters" section for that learner.

In addition, **every keyword argument accepted by** ``__init__`` **should
correspond to an attribute on the instance**. dodiscover relies on this to
find the relevant attributes to set on an learner when doing model selection.
...
make it possible to use the learner as part of a pipeline that can
mix both supervised and unsupervised transformers, even unsupervised
learners need to accept a ``y=None`` keyword argument in
the second position that is just ignored by the learner.
...
general, calling ``learner.fit(X1)`` and then ``learner.fit(X2)`` should
be the same as only calling ``learner.fit(X2)``. However, this may not be
true in practice when ``fit`` depends on some random process, see
:term:`random_state`. Another exception to this rule is when the
hyper-parameter ``warm_start`` is set to ``True`` for learners that
support it. ``warm_start=True`` means that the previous state of the
trainable parameters of the learner are reused instead of using the
...
some regression learner would be stored in a ``coef_`` attribute after
``fit`` has been called.
...
learners that expect tabular input should set a `n_features_in_`
attribute at `fit` time to indicate the number of features that the learner
expects for subsequent calls to `predict` or `transform`.

.. _rolling_your_own_learner:

Rolling your own learner
==========================
If you want to implement a new learner that is dodiscover-compatible,
whether it is just for you or for contributing it to dodiscover, there are
several internals of dodiscover that you should be aware of in addition to
the dodiscover API outlined above. You can check whether your learner
adheres to the dodiscover interface and standards by running
:func:`~dodiscover.utils.learner_checks.check_learner` on an instance. The
:func:`~dodiscover.utils.learner_checks.parametrize_with_checks` pytest
decorator can also be used (see its docstring for details and possible
interactions with `pytest`)::

  >>> from dodiscover.utils.learner_checks import check_learner
  >>> from dodiscover.svm import LinearSVC
  >>> check_learner(LinearSVC())  # passes

The main motivation to make a class compatible to the dodiscover learner
...
.. topic:: ``Baselearner`` and mixins:

    We tend to use "duck typing", so building an learner which follows
    the API suffices for compatibility, without needing to inherit from or
    even import any dodiscover classes.

    However, if a dependency on dodiscover is acceptable in your code,
    you can prevent a lot of boilerplate code
    by deriving a class from ``Baselearner``
    and optionally the mixin classes in ``dodiscover.base``.
    For example, below is a custom constraint-based learner.

      >>> import numpy as np
      >>> from dodiscover.base import Baselearner, ConstraintMixin
      >>> from dodiscover.utils.validation import check_data_context
      >>> from dodiscover.metrics import euclidean_distances
      >>>
      >>> class TemplateLearner(Baselearner, ConstraintMixin):
      ...
      ...
      ...
      ...         # Check that data and context are consistent
      ...         data, context = check_data_context(data, context)
      ...
      ...         self.graph_ = context.init_graph
      ...         # Return the learner
      ...         return self
      ...
...
-------------------------
All dodiscover learners have ``get_params`` and ``set_params`` functions.
The ``get_params`` function takes no arguments and returns a dict of the
``__init__`` parameters of the learner, together with their values.
...
sub-learners (for most learners, this can be ignored). The default value
for ``deep`` should be `True`. For instance considering the following
learner::

    >>> from dodiscover.base import Baselearner
    >>> from dodiscover.linear_model import LogisticRegression
    >>> class Mylearner(Baselearner):
    ...     def __init__(self, sublearner=None, my_extra_param="random"):
    ...         self.sublearner = sublearner
    ...         self.my_extra_param = my_extra_param

The parameter `deep` will control whether or not the parameters of the
`sublearner` should be reported. Thus when `deep=True`, the output will be::

    >>> my_learner = Mylearner(sublearner=LogisticRegression())
    >>> for param, value in my_learner.get_params(deep=True).items():
    ...     print(f"{param} -> {value}")
    my_extra_param -> random
    sublearner__C -> 1.0
    sublearner__class_weight -> None
    sublearner__dual -> False
    sublearner__fit_intercept -> True
    sublearner__intercept_scaling -> 1
    sublearner__l1_ratio -> None
    sublearner__max_iter -> 100
    sublearner__multi_class -> auto
    sublearner__n_jobs -> None
    sublearner__penalty -> l2
    sublearner__random_state -> None
    sublearner__solver -> lbfgs
    sublearner__tol -> 0.0001
    sublearner__verbose -> 0
    sublearner__warm_start -> False
    sublearner -> LogisticRegression()

Often, the `sublearner` has a name (as e.g. named steps in a
:class:`~dodiscover.pipeline.Pipeline` object), in which case the key should
become `<name>__C`, `<name>__class_weight`, etc.

While when `deep=False`, the output will be::

    >>> for param, value in my_learner.get_params(deep=False).items():
    ...     print(f"{param} -> {value}")
    my_extra_param -> random
    sublearner -> LogisticRegression()
...
``'parameter': value`` and sets the parameters of the learner using this dict.
Return value must be the learner itself.
...
``__repr__`` method, is to inherit from ``dodiscover.base.Baselearner``. If you
do not want to make your code dependent on dodiscover, the easiest way to
implement the interface is::

    def get_params(self, deep=True):
        # suppose this learner has parameters "alpha" and "recursive"
...
to apply parameter setting to learners,
...
All logic behind learner parameters,
...
trailing ``_`` is used to check if the learner has been fitted.
...
an learner must support the ``base.clone`` function to replicate an learner.
This can be done by providing a ``get_params`` method.
If ``get_params`` is present, then ``clone(learner)`` will be an instance of
``type(learner)`` on which ``set_params`` has been called with clones of
the result of ``learner.get_params()``.
...
learners can customize the behavior of :func:`base.clone` by defining a
`__dodiscover_clone__` method. `__dodiscover_clone__` must return an instance of the
learner. `__dodiscover_clone__` is useful when an learner needs to hold on to
some state when :func:`base.clone` is called on the learner. For example, a
frozen meta-learner for transformers can be defined as follows::

    class FrozenTransformer(Baselearner):
        ...
        def __dodiscover_clone__(self):
            return self

        def fit(self, X, y):
            # Fitting does not change the state of the learner
            return self
...
----------------------
For an learner to be usable together with ``pipeline.Pipeline`` in any but the
...
learner types
---------------
Some common functionality depends on the kind of learner passed.
...
is implemented using the ``_learner_type`` attribute, which takes a string value.
...
will set the attribute automatically.  When a meta-learner needs to distinguish
among learner types, instead of checking ``_learner_type`` directly, helpers
...
independent term is stored in ``intercept_``.  ``dodiscover.linear_model._base``
contains a few base classes and mixins that implement common linear model
patterns.

The :mod:`dodiscover.utils.multiclass` module contains useful functions
...
.. warning::
    ...
    The learner tags are experimental and the API is subject to change.

dodiscover introduced learner tags in version 0.21. These are annotations
of learners that allow programmatic inspection of their capabilities, such as
sparse matrix support, supported output types and supported methods. The
learner tags are a dictionary returned by the method ``_get_tags()``. These
tags are used in the common checks run by the
:func:`~dodiscover.utils.learner_checks.check_learner` function and the
:func:`~dodiscover.utils.learner_checks.parametrize_with_checks` decorator.
Tags determine which checks to run and what input data is appropriate. Tags
can depend on learner parameters or even system architecture and can in
general only be determined at runtime.

The current set of learner tags are:

allow_nan (default=False)
    whether the learner supports data with missing values encoded as np.nan

binary_only (default=False)
    whether learner supports binary classification but lacks multi-class
    classification support.

multilabel (default=False)
    whether the learner supports multilabel output
...
multioutput_only (default=False)
    whether learner supports only multi-output classification or regression.

no_validation (default=False)
    whether the learner skips input-validation. This is only meant for
    stateless and dummy transformers!

non_deterministic (default=False)
    whether the learner is not deterministic given a fixed ``random_state``
...
pairwise (default=False)
    ...
    learner has a `metric` or `affinity` or `kernel` parameter with value
    'precomputed'. Its primary purpose is to support a :term:`meta-learner`
    or a cross validation procedure that extracts a sub-sample of data intended
    for a pairwise learner, where the data needs to be indexed on both axes.
    Specifically, this tag is used by
    :func:`~dodiscover.utils.metalearners._safe_split` to slice rows and
    columns.
...
poor_score (default=False)
    whether the learner fails to provide a "reasonable" test-set score, which
    ...
    are based on current learners in dodiscover and might be replaced by
    something more systematic.

requires_fit (default=True)
    whether the learner requires to be fitted before calling one of
    `transform`, `predict`, `predict_proba`, or `decision_function`.

requires_positive_X (default=False)
    whether the learner requires positive X.

requires_y (default=False)
    whether the learner requires y to be passed to `fit`, `fit_predict` or
    `fit_transform` methods. The tag is True for learners inheriting from
    `~dodiscover.base.RegressorMixin` and `~dodiscover.base.ClassifierMixin`.

requires_positive_y (default=False)
    whether the learner requires a positive y (only applicable for regression).
...
_xfail_checks (default=False)
    ...
    :func:`~dodiscover.utils.learner_checks.parametrize_with_checks`. These
    checks will be simply ignored and not run by
    :func:`~dodiscover.utils.learner_checks.check_learner`, but a
    `SkipTestWarning` will be raised.
    Don't use this unless there is a *very good* reason for your learner
...
stateless (default=False)
    whether the learner needs access to data for fitting. Even though an
    learner is stateless, it might still need a call to ``fit`` for
    initialization.

X_types (default=['2darray'])
    Supported input types for X as list of strings. Tests are currently only
    run if '2darray' is contained in the list, signifying that the learner
...
In addition to the tags, learners also need to declare any non-optional
parameters to ``__init__`` in the ``_required_parameters`` class attribute,
which is a list or tuple.  If ``_required_parameters`` is only
``["learner"]`` or ``["base_learner"]``, then the learner will be
instantiated with an instance of ``LogisticRegression`` (or
``RidgeRegression`` if the learner is a regressor) in the tests. The choice
...
With
`SLEP018 <https://dodiscover-enhancement-proposals.readthedocs.io/en/latest/slep018/proposal.html>`__,
dodiscover introduces the `set_output` API for configuring transformers to
...
when defining a custom subclass::
    ...
    class MyTransformer(TransformerMixin, Baselearner, auto_wrap_output_keys=None):
...
inclusion in dodiscover, and which may be appropriate to adopt in external
...
dodiscover project tries to closely follow the official Python guidelines
...
* Use relative imports for references inside dodiscover.

* Unit tests are an exception to the previous rule;
  they should use absolute imports, exactly as client code would.
  A corollary is that, if ``dodiscover.foo`` exports a class or function
  that is implemented in ``dodiscover.foo.bar.baz``,
  the test should import it from ``dodiscover.foo``.
...
.. currentmodule:: dodiscover.utils

The module :mod:`dodiscover.utils` contains various functions for doing input
...
passed to a dodiscover API function. The exact parameters to use depends
...
See :func:`dodiscover.utils.check_random_state` in :ref:`developers-utils`.

Here's a simple example of code using some of the above guidelines::

    from dodiscover.utils import check_array, check_random_state
...
If you use randomness in an learner instead of a freestanding function,
some additional guidelines apply.

First off, the learner should take a ``random_state`` argument to its
...
The following example should make this clear::
    ...
    class GaussianNoise(Baselearner, TransformerMixin):
        """This learner ignores its input and returns random Gaussian noise.

        It also does not adhere to all dodiscover conventions,
...
when an learner is ``fit`` twice to the same data,
...
do use :func:`dodiscover.utils._testing.assert_allclose`.
...
:func:`dodiscover.utils._testing.assert_allclose`.
